...
class Population(object):
    ...
    def report_average(self,
        ...
        for id in self.population:
            ...
            if not value1 in outbreaksize:
                outbreaksize[value1] = 0
                total[value1] = 0
            outbreaksize[value1] += int(value2)
            total[value1] += 1
        ...
        for i in range(0,vacc_size):
            cur_val = values[i]
            average += (float(outbreaksize[cur_val])/float(total[cur_val]))
...
pop.read_data("./analyses/average_time_by_infections_perc.csv",
              "contacts",3)
pop.read_data("./analyses/degree10min_perc_nozeros.csv",
              "time",4)
